...
LOG_FILE_SUFFIX = 'process_log_OCR.csv'
...
def draw_random_subdir(processedDir, logDir):
    ...
    if not os.path.exists(logDir + LOG_FILE_SUFFIX):
        return directory, subdirectory
    df_log = pd.read_csv(logDir + LOG_FILE_SUFFIX)
...
while not stop_condition:
    ...
    if os.path.exists(logDir + LOG_FILE_SUFFIX):
        df_log = pd.read_csv(logDir + LOG_FILE_SUFFIX)
    ...
    img_fns = [
        processedDir + subdir_path_end + file
    ...
    if len(df_result) > 0 and len(df_read) > 0:
    ...
    for i in range(len(df_merge)):
        if df_merge['processed_image_class'].iloc[i] == 'loss':
            if 'details' in df_merge.columns and df_merge['details'].iloc[i] == 'OCR read metadata contains letters':
                for col in md_cols:
    ...
    print(f'Processing rate: {str(round(t / len(img_fns), 2))} s/img')
    print('')
    ...
    if os.path.exists(logDir + LOG_FILE_SUFFIX):
        ...
        df_update.to_csv(logDir + LOG_FILE_SUFFIX, index=False)
    elif len(df_result_) > 0:
        df_result_.to_csv(logDir + LOG_FILE_SUFFIX, index=False)
